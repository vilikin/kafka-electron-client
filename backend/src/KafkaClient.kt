...
data class KafkaClientStateConnected(
  ...
  val refreshTopicOffsetsTimer: Timer,
...
class KafkaClient {
  ...
  fun connect(
    ...
    if (state is KafkaClientStateDisconnected) {
      ...
      try {
        ...
        val refreshTopicOffsetsTimer = fixedRateTimer(initialDelay = 500, period = 5_000) {
          refreshTopicOffsets()
        }
  ...
  private fun refreshTopicOffsets() {
    (state as? KafkaClientStateConnected)?.let { connectedState ->
      val topicPartitions =
        connectedState.adminClient.describeTopics(connectedState.subscribedToOffsetsOfTopics)
          .all()
          .get()
          .values
          .flatMap { topic ->
      ...
      val earliestOffsets = connectedState.adminClient.listOffsets(requestForEarliestOffsets)
        .all()
        .get()

      val latestOffsets = connectedState.adminClient.listOffsets(requestForLatestOffsets)
        .all()
        .get()

      val result = (earliestOffsets.keys + latestOffsets.keys)
        .map {
          PartitionOffsets(
            it.topic(),
            it.partition(),
      ...
      broadcast(RefreshTopicOffsets(result))
    }
  }
